...
# ultichain_hash
RUN git clone https://github.com/ultichainpay/ultichain_hash
RUN cd ultichain_hash && python3 setup.py install
...
RUN mkdir /ultichain-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /ultichain-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /ultichain-src
