...
from test_framework.mininode import ultichainhash
from test_framework.util import (assert_equal,
                                 bytes_to_hex_str,
                                 hash256,
                                 )

def ultichainhash_helper(b):
    return encode(ultichainhash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def _zmq_test(self):
        ...
        for x in range(num_blocks):
            ...
            assert_equal(genhashes[x], ultichainhash_helper(block[:80]))
