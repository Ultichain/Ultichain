...
const char * const BITCOIN_CONF_FILENAME = "ultichain.conf";
const char * const BITCOIN_PID_FILENAME = "ultichaind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ultichaind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ultichain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ultichain
    // Mac: ~/Library/Application Support/Ultichain
    // Unix: ~/.ultichain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ultichain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ultichain";
#else
    // Unix
    return pathRet / ".ultichain";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty ultichain.conf if it does not excist
